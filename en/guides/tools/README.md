...
Tools can extend the capabilities of LLMs (Language Learning Models), such as performing web searches, scientific calculations, or generating images, thereby enhancing the LLM's ability to connect with the external world. AgentBase provides two types of tools: **First-party Tools** and **Custom Tools**.

You can directly use the first-party built-in tools provided by the AgentBase ecosystem, or easily import custom API tools (currently supporting OpenAPI / Swagger and OpenAI Plugin specifications).

#### Functions of Tools:

1. Tools allow users to create more powerful AI applications on AgentBase. For example, you can arrange suitable tools for an intelligent assistant application (Agent) that can complete complex tasks through task reasoning, step-by-step breakdown, and tool invocation.
...
{% hint style="info" %}
We welcome you to contribute your developed tools to AgentBase. For detailed methods on how to contribute, please refer to the [AgentBase Development Contribution Documentation](https://github.com/agent-base/agentbase/blob/main/CONTRIBUTING.md). Your support is invaluable to us.
{% endhint %}

#### First-party Tool Authorization

If you need to use the first-party built-in tools provided by the AgentBase ecosystem, you need to configure the corresponding credentials before using them.
...
You can also use [agentbase-tools-worker](https://github.com/crazywoola/agentbase-tools-worker) to quickly deploy custom tools. This tool provides:

* Routes that can be imported into AgentBase `https://agentbasetoolsworker.yourname.workers.dev/doc`, offering an OpenAPI-compatible interface documentation.
