...
1. Before using the external data tool, you need to prepare an API and an API Key for authentication. Please read [External Data Tool](https://docs.agentbase.ai/guides/extension/api-based-extension/external-data-tool)
2. AgentBase provides centralized API management. After adding the API extension configuration in the settings interface, it can be used directly in various applications on AgentBase.
