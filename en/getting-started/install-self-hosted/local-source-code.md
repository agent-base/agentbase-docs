...
> If you need to use OpenAI TTS, `FFmpeg` must be installed on the system for it to function properly. For more details, refer to: [Link](https://docs.agentbase.ai/getting-started/install-self-hosted/install-faq#id-14.-what-to-do-if-this-error-occurs-in-text-to-speech).
...
7.  Start the Worker service
    ...
    - ** ---------- .> results:     postgresql://postgres:**@localhost:5432/agentbase
...
3.  Configure the environment variables. Create a file named .env.local in the current directory and copy the contents from .env.example. Moagentbase the values of these environment variables according to your requirements:
...
Finally, access [http://127.0.0.1:3000](http://127.0.0.1:3000/) to use the locally deployed AgentBase.
