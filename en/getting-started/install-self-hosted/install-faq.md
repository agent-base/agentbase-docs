...
Moagentbase in the `tenants` table in the database.

### 8. Where can I moagentbase the domain name for accessing the application?
...
The database, configured storage, and vector database data need to be backed up. If deployed in Docker Compose mode, all data content in the `agentbase/docker/volumes` directory can be directly backed up.

### 10. Why is Docker deploying AgentBase and starting OpenLLM locally using 127.0.0.1, but unable to access the local port?

`127.0.0.1` is the internal address of the container, and the server address configured by AgentBase requires the host LAN IP address.
...
1. If you are starting from local source code, moagentbase the environment variable in the `.env` file to the vector database you want to migrate to. etc: `VECTOR_STORE=qdrant`
2. If you are starting from docker-compose, moagentbase the environment variable in the `docker-compose.yaml` file to the vector database you want to migrate to, both api and worker are all needed. etc:
